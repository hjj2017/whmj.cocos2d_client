 ...
 */
@ccclass
export default class JoinRoomDialogComp extends cc.Component {
    /**
     * 当前数字
     */
    _strCurrNum: string = "";
 ...
 * @param SELF this 指针
 */
function __regUIEvent(SELF: JoinRoomDialogComp): void {
    if (null == SELF) {
        return;
    }
    ...
    // "加入" 按钮点击事件
    cc.find("PrimeDialog/Center/ButtonArea/Button_JoinRoom_", SELF.node)
        .on(cc.Node.EventType.TOUCH_END, (/*oEvent*/) => {
            if (null != SELF.onJoinRoom) {
                SELF.onJoinRoom(Number.parseInt(SELF._strCurrNum));
            }
        }
    );
}
 ...
 * @param SELF this 指针
 */
function __updateCurrNumDisplay(SELF: JoinRoomDialogComp): void {
    if (null == SELF) {
        return;
    }
