...
1 目录下也有 img 目录，且这个 img 目录下也有 AutoAtlas.pac。

两个等级的资源，分别用不同的合图配置，这样做的目的也是为了加快场景的加载速度。

如果场景不复杂，但是却和其他 prefab 公用一张合图，那会极大的影响加载速度。

这里有个问题，如果我们遇到 Button.png 即在场景里用到了，又在 prefab 里用到了，那么这个图片到底是应该放到 0 里还是 1 里？

XXX 注意：这种情况 0 和 1 各放一份！

虽然冗余了，但是这却避免了出现小场景或小 prefab 加载大图片资源的问题。

另外 Cocos Creator 也在内部尝试了框架机制的优化。

例如 Button.png 只放到 0 里，但是场景和 prefab 都用到了，那么 Cocos Creator 在创建合图时，很可能会创建一个零散的碎图……
